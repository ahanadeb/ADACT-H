...
# def get_candidates_ao(Q_prev, pdfa):
#     Q_t = []
#     for q in Q_prev:
...
def test_distinct(q1, q2, o, H, thres):
    ...
    #     L=
    #     Data = q1.Data[:,q1.t]
    L = get_suffixes_bare(o, q1)
    q1.ix_new = L
    print(q1.name, q2.name)


    q1_prob= q1.operatorC13o()
    if len(q1_prob)==0:
        return True, False
    # r, o = q2.operatorC11()
    ...
    for s in seq:
        ...
        if np.abs(p1 - p2) > thres:
            print("promoted ", q2.name)
            return False, True
    return True, True


def test_distinct2(q1,q2, thres):
    # if q2.t == H + 1:
    #     return True
    #r, o = q1.operatorC11()
...
def get_suffixes23(q, o, u, ao):
    t = u.t
    if q=='q0':
        L = []
        for i in u.ix:
            if u.Data[i, t ][0] == ao[0] and u.Data[i, t ][1] == ao[1]:
                L.append(i)
        return L
    L = []
    for i in u.ix:
